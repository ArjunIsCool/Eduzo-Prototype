...
{
    public class TablesMenuUIManager : MonoBehaviour
    {
        ...
        TablesGameSettings chosenGameSettings;

        public static TablesMenuUIManager Instance;
        ...
        private void Start()
        {
            chosenGameSettings = new TablesGameSettings();
            InitializeOptions();
        }
        ...
        void InitializeOptions()
        {
            ...
            for (int i = 1; i <= TablesGlobalConstants.TOTAL_TABLES; i++)
            {
        ...
        public void OnChangeGameMode(bool isPracticeMode)
        {
            if (isPracticeMode)
            {
                chosenGameSettings.gameMode = TablesGameSettings.GameMode.PRACTICE;
                timeInputField.gameObject.SetActive(false);
                chosenGameSettings.totalTimeSeconds = 9999999;
            }
            else
            {
                chosenGameSettings.gameMode = TablesGameSettings.GameMode.TEST;
        ...
        public void UpdateQuestionsDropDown()
        {
            ...
            for (int i = 0; i < TablesGlobalConstants.TOTAL_POSSIBLE_QUESTIONS; i++)
            {
                GameObject questionItem = Instantiate(questionItemPrefab, questionsHolder);
                questionItem.GetComponent<TablesQuestionItem>().Initalize(chosenGameSettings.tablesNo, i + 1);
            }
        }
        ...
        public TablesGameSettings GetGameSettings()
        {
