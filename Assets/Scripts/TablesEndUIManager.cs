...
{
    public class TablesEndUIManager : MonoBehaviour
    {
        ...
        public static TablesEndUIManager Instance;
        ...
        public void UpdateEndScreenInfo(int scorePercentage)
        {
            ...
            for (int i = 2; i >= 0; i--)
            {
                if (i < stars)
                {
                    starsHolder.GetChild(i).GetComponent<TablesStar>().SetStar(true);
                }
                else
                {
                    starsHolder.GetChild(i).GetComponent<TablesStar>().SetStar(false);
                }
            }
        }
